	...
	<view>
		<view class="wrap">
			<view class="u-tabs-box">
				<u-tabs-swiper activeColor="#FF4544" ref="tabs" :list="tabs" :current="current" @change="change" :is-scroll="false" swiperWidth="750"></u-tabs-swiper>
			</view>
			<swiper class="swiper-box" :current="swiperCurrent" @transition="transition" @animationfinish="animationfinish">
							...
							<view class="order" v-for="(res, index) in orderList[0]" :key="index">
								<view class="top">
									<view class="left">
										...
										<view class="store">{{ res.order_no }}</view>
										<u-icon name="arrow-right" color="rgb(203,203,203)" :size="26"></u-icon>
									</view>
									<view class="right">{{res.create_time | date("yyyy-mm-dd hh:MM:ss")}}</view>
								</view>
								<view class="item" v-for="(item, index) in res.order_goods" :key="index">
									<view class="left">
										<image :src="getImgUrl(item.sku_image)" mode="aspectFill"></image>
									</view>
									<view class="content">
										<view class="title u-line-2">{{ item.sku_name }}</view>
										<!-- <view class="type">数量: {{ item.num }} <text class="u-m-l-40">单价 {{ item.price }}</text></view>
										<view class="delivery-time">价格 {{ item.price }}</view> -->
									</view>
									<view class="right">
										<view class="price">
											<!-- ￥{{ priceInt(item.price) }} -->
				...
				<swiper-item class="swiper-item">
					<scroll-view scroll-y style="height: 100%;width: 100%;" @scrolltolower="reachBottom">
						<view class="page-box">
							<view class="order" v-for="(res, index) in orderList[4]" :key="index">
								<view class="top">
									<view class="left">
										<u-icon name="order" :size="30" color="rgb(94,94,94)"></u-icon>
										<view class="store">{{ res.order_no }}</view>
...
export default {
	...
	onLoad(e) {
		let index = e.index || 0
		this.current = index;
		this.orderStatus = this.tabs[index].status;
	...
	methods: {
		...
		getOrder(index) {
			...
			this.$u.api.orderList(param).then(res => {
				if (res.code == 0) {
					var list = res.data.list;
					if(list.length === 0 || list.length < this.pageConfig.page_size) {
						this.loadStatus[index] = 'nomore';
						// this.$set(this.loadStatus,index,'nomore');
						return;
					}
					// mr.sui 这里有个问题,js无法对数组索引进行响应式变化, Vue.set(object|array, key, value) / vm.$set()
					//this.$forceUpdate(); 重新渲染组件也可以解决
					var temp = this.orderList[index].concat(list)
